...
def find_piece_header_row(rows: List[List[str]]) -> Optional[int]:
    ...
    for r_idx, row in enumerate(rows):
        s = {cell.strip() for cell in row if cell is not None}
...
def find_crew_info_table(rows: List[List[str]]) -> Optional[Tuple[int, int, int, dict]]:
    ...
    where start_row_idx is the first data row after the header and end_row_idx_exclusive
    is the first row AFTER the table (typically a '#ERROR!' marker).

    The Crew Info header row is identified by containing at least:
    ...
    for r_idx, row in enumerate(rows):
        ...
        if required.issubset(s):
            ...
            while end < len(rows):
                first_cell = (rows[end][0] if len(rows[end]) > 0 else "") or ""
                if first_cell.strip() == "#ERROR!":
                    break
                end += 1
...
def sanitize_token(s: str) -> str:
    ...
    - Trim
    - Convert internal whitespace to single underscores
...
def extract_yyyymmdd(rows: List[List[str]]) -> str:
    # Tries to read from "Start Time" cell, else falls back to today's date.
...
def build_output_filename(season: str, rows: List[List[str]], shell: str, zone: str, piece: str, piece_num: str) -> str:
    """Build output filename per protocol:

    Season_YYYYMMDD_BoatName_Zone_Piece_PieceNumber.csv

    - Season defaults to FY26 if blank.
    - BoatName is uppercased.
    - Zone is uppercased.
    - PieceNumber is coerced to an integer string.
    - Date comes from the CSV Start Time when possible, else today's date.
    """
    ...
    # Coerce piece number to int string
    try:
...
def apply_updates(
    ...
    # Write weights and enforce seats 1–8 not blank
    for r in range(crew_start, crew_end):
        ...
        # Prefer abbr-based, fall back to seat-based
    ...
    header = rows[piece_header_r]
    cols = header_col_map(header)

    comment_c = cols["comment"]
    wind_c = cols["Wind"]
    stream_c = cols["Stream"]
    validated_c = cols["Validated"]

    # Insert Zone column right of comment (between comment and Wind)
    zone_c = comment_c + 1

    # After insertion, shift known column indices to the right if they were after comment
    if wind_c > comment_c:
        wind_c += 1
    if stream_c > comment_c:
        stream_c += 1
    if validated_c > comment_c:
        validated_c += 1

    # Temperature column immediately right of Validated (after shifting)
    temp_c = validated_c + 1

    # Insert the Zone column for the header row and each piece data row until the next '#ERROR!'
    rr = piece_header_r
    while rr < len(rows):
        first_cell = (rows[rr][0] if len(rows[rr]) > 0 else "") or ""
        if rr != piece_header_r and first_cell.strip() == "#ERROR!":
            break
        pad_row(rows[rr], zone_c)
        rows[rr].insert(zone_c, "")
        rr += 1

    # Set header name for Zone
    pad_row(rows[piece_header_r], zone_c + 1)
    rows[piece_header_r][zone_c] = "Zone"

    # Ensure header has Temperature at temp_c
    pad_row(rows[piece_header_r], temp_c + 1)
    rows[piece_header_r][temp_c] = "Temperature"

    # Fill piece rows
    r = piece_header_r + 1
    while r < len(rows):
        first_cell = (rows[r][0] if len(rows[r]) > 0 else "") or ""
        if first_cell.strip() == "#ERROR!":
            break
        if any((cell or "").strip() for cell in rows[r]):
